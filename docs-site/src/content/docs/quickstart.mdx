...
Open `grading.json` for any case to see each assertion's verdict and evidence in [Anthropic's published format](https://platform.claude.com/docs/en/agents-and-tools/agent-skills):
...
- **[CLI reference](/arc-skill-eval/cli-reference/).** See every `arc-skill-eval run` flag.
- **[Runtime and models](/arc-skill-eval/runtime-and-models/).** Configure model pinning, Pi defaults, GPT 5.5, and Ollama Cloud.
- **[Authoring evals](/arc-skill-eval/authoring-evals/).** Write `evals/evals.json` for your own skill.
- **[Dogfooding and authoring loop](/arc-skill-eval/dogfooding/).** Iterate with `--compare` and run artifacts.
- **[Hello-world example](/arc-skill-eval/examples/hello-world/).** Walk through the skill you just ran.
