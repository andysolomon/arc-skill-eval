...
description: Skills ship as markdown plus optional scripts and resist every form of change-detection we've built for code. Evals. and the with/without skill delta in particular. are the repeatable evidence about how a skill behaves before and after a change.
pubDate: 2026-04-28
author: Andy Solomon
draft: false
sidebar:
...
This is the part `arc-skill-eval` (and Anthropic's published methodology) reports as the primary metric. A case that passes 100% of the time *with* the skill attached is meaningless if it would also pass 100% of the time *without* it. The model would have done the right thing on its own; the skill contributed nothing.
