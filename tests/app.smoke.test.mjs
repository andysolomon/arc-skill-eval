...
test('typing a filter narrows the case list', async () => {
  ...
  await waitFor((f) => /Skills/.test(f));
  await sleep(150);                // let ink's useInput effect subscribe to stdin
  stdin.write('/');                // enter filter mode
  assert.ok(await waitFor(filterEngaged), 'filter prompt should appear');
  ...
  assert.ok(await waitFor((f) => /\/ demo/.test(f)), 'filter prompt should echo the typed query');
  unmount();
});
