# arc-skill-eval

Pi-native framework and CLI for evaluating skills that ship adjacent `SKILL.md` and `skill.eval.ts` files.

## Status
Bootstrapping repo foundation, planning artifacts, contract loading/normalization, initial Pi SDK runner + session telemetry support, and semantic-release automation.

## Core docs
- `docs/skill-evals-v1.md`
- `docs/skill-eval-schema.md`
- `docs/framework-repo-structure.md`
- `docs/domain-model.md`
